...
const Home = () => {
  ...
  const handleAddIcon = async () => {
    ...
    const savedIcon = { ...newIcon, id: Date.now(), category: "General" };
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: isDark?"#111":"#f8f8f8" }]}>
      ...
      {/* Row: Search + Time + Connector + Add + Speak */}
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginVertical:10 }}>
        <View style={styles.controlsRow}>
          ...
          <TextInput
            ...
            style={[styles.searchInput, { backgroundColor:isDark?"#333":"#fff", color:isDark?"#fff":"#000" }]}
          />

          {/* Time Picker */}
          <View style={[styles.pickerBox, { backgroundColor:isDark?"#333":"#f0f0f0" }]}>
            <Picker
              selectedValue={timeOption}
              style={{ height: 40, width: 130, color:isDark?"#fff":"#000" }}
              onValueChange={(itemValue) => setTimeOption(itemValue)}
            >
              {timeOptionsByLang[lang].map((t,i) => <Picker.Item key={i} label={t} value={t} />)}
            </Picker>
          </View>

          {/* Single Connector Picker */}
          <View style={[styles.pickerBox, { backgroundColor:isDark?"#333":"#f0f0f0" }]}>
            <Picker
              selectedValue={connector}
              style={{ height: 40, width: 100, color:isDark?"#fff":"#000" }}
              onValueChange={(itemValue) => setConnector(itemValue)}
            >
              {connectorOptionsByLang[lang].map((c,i) => <Picker.Item key={i} label={c} value={c} />)}
            </Picker>
          </View>
...
const styles = StyleSheet.create({
  ...
  searchInput:{ width:150, borderWidth:1, borderRadius:8, padding:8, marginRight:5 },
  pickerBox:{ borderRadius:8, marginRight:5, overflow:"hidden" },
  btnRow:{ backgroundColor:"#0d6efd", paddingVertical:12, paddingHorizontal:16, borderRadius:8, justifyContent:"center", alignItems:"center", marginLeft:5 },
